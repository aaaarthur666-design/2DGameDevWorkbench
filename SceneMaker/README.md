...
一个完全在浏览器中运行的 2D 地图拼接、分层编辑、扩图和游戏引擎资源导出工具。图片默认只停留在当前浏览器内存中；除非主动启用“外部图片 API”，不会上传到服务器。
...
浏览器访问终端显示的地址，通常为 `http://localhost:3000/`。
...
在“设置 → 外部图片 API”中填写地址和临时令牌。SceneMaker 会发送：
...
服务响应需包含 `image`、`data` 或 `url` 之一。图片值可以是 Data URL、纯 Base64 或可公开读取的 HTTPS URL。令牌仅保存在当前页面内存，并通过 `Authorization: Bearer <token>` 发送。
