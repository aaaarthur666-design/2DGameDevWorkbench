...
def test_new_review_preserves_human_marks_and_retains_current_issues(setup):
    ...
    # A human can use the same note as the automatic review; provenance matters.
    ...
    human=s.get_job(job.job_id).candidates[0].frames[9].model_dump()
    control._mark(job.job_id,1,report,"still-failing")
    assert s.get_job(job.job_id).candidates[0].frames[9].model_dump()==human
    ...
    assert candidate.frames[9].model_dump()==human
