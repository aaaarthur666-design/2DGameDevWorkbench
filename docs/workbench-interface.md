...
外层工作台围绕序列帧与场景组织资产生产，保持已有序列帧、地图、交互物工具的内部编辑方式。它是外部 Agent 的可视协作面：展示真实任务、进度和产物，并承载画布、图层、碰撞区等不适合纯对话操作的内容；页面不提供通用 Agent 对话框。

界面采用深蓝黑、网格和明确的序列帧/场景色彩语义，不再使用仿 macOS 的窗口灯或系统标题栏装饰。中文操作名优先，协议参数集中到工具内部或高级页。
...
首次进入开始页展示三步引导：选择序列帧或场景、了解首件作品所需素材、学习从制作记录继续。支持上一步、跳过、最终进入所选路线。步骤和完成状态保存在当前浏览器的 `workbench.onboarding.v1`；顶部「新手引导」和高级工具中的入口均可从第一步重看。
