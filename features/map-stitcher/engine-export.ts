...
import { canUseSeparatedComposite } from './map-production';
import {
  ...
  return {
    ...
    regions: shapes.flatMap((shape) => {
      ...
      return [
        {
          ...shape,
          points: mapShapeToWorldPixels(shape, tile, sourceWidth, sourceHeight),
        },
      ];
    }),
  };
...
export function buildGodotScene(
  manifest: RegionExportManifest,
  layers: Array<MapDisplayLayer | 'top'>,
  useSeparatedComposite = layers.includes('surface') &&
    layers.includes('object'),
) {
  const extResources = layers.map(
    (layer, index) =>
      `[ext_resource type="Texture2D" path="res://assets/map_${layer}.png" id="${index + 1}_${layer}"]`,
  );
  const collisionRegions = manifest.regions.filter(
    (region) => region.layer === 'collision' && region.points.length >= 3,
  );
  const nodes = layers.flatMap((layer, index) => {
    if (layer === 'top') return [];
    const zIndex = layer === 'object' ? 20 : layer === 'mask' ? 10 : 0;
    const visible = useSeparatedComposite
      ? layer === 'surface' || layer === 'object'
      : layer === 'overall';
    return [
      [
...
export async function exportGodotPackage(
  ...
  const layerCandidates: Array<MapDisplayLayer | 'top'> = [
    'overall',
    'surface',
    'object',
    'mask',
  ...
  const manifest = buildRegionManifest(
    tiles,
    shapes,
    sourceWidth,
    sourceHeight,
  ...
  zip.file(
    'map_scene.tscn',
    buildGodotScene(manifest, exportedLayers, canUseSeparatedComposite(tiles)),
  );
  zip.file('frame_ronin_regions.gd', buildGodotRegionRuntime());
  if (snapshot)
    zip.file(
      'source_state.zip',
      (await createPixelworkStatePackage(snapshot)).blob,
    );
  zip.file(
    'INSTALL.md',
...
function shouldExportLayer(
  ...
  if (layer === 'mask')
    return tiles.some(
      (tile) => !tile.hidden && tile.images.overall && tile.images.object,
    );
  if (layer === 'top')
    return shapes.some(
      (shape) =>
        shape.layer === 'top' &&
        tiles.some((tile) => !tile.hidden && tile.key === shape.tileKey),
    );
  return tiles.some((tile) => !tile.hidden && Boolean(tile.images[layer]));
}
