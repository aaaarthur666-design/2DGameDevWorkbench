...
export const LEGACY_SIDE_VIEW_PROMPT = `Extend the supplied artwork into a background for a strictly 2D side-scrolling platformer.
...
export const DEFAULT_OVERALL_PROMPT = `Extend the supplied artwork into a background for a strictly 2D side-scrolling platformer. This is a continuation of the same scene, not a new composition.
Use a straight-on orthographic side view, like a flat side elevation. The image axes represent horizontal movement and vertical height, not depth. Keep a constant object scale. Do not use a top-down plan view, elevated or angled camera, isometric projection, perspective convergence, receding floor surfaces, or 3D rendering. Preserve the reference's stylized 2D shading.
GEOMETRY CONTINUITY IS THE PRIORITY: Identify the main walkable ground surface where it meets the existing artwork, distinguishing it from balconies, rooftops, wall ledges and background scenery. When extending left or right, continue that ground surface at exactly the same vertical position in the canvas across the newly generated area. For an existing level street, keep it level all the way across; do not raise or lower it after the overlap ends or after completing a building. Preserve the thickness of the ground and foundation below it. Do not invent steps, raised platforms, retaining walls, thick foundations or a new ground level unless the user explicitly requests them. Continue an existing slope at its established direction rather than flattening it. If visible references disagree, connect their existing elevations without moving them; do not invent another elevation. When extending upward or downward, or when no ground surface is visible in the reference, continue only the existing scene structures and do not insert a new ground strip. Do not allocate a fixed fraction of the image to ground, floor, sky or buildings. Preserve the reference ground height and occupied proportion instead of composing a new scene.
The input is an outpainting template: existing artwork may occupy any overlapping portions, not necessarily every edge. Fill only fully transparent pixels. Preserve all existing non-transparent artwork, including partially transparent edge pixels, without repainting, shifting, scaling or mirroring it. Return the same canvas dimensions and aspect ratio, with no crop, border or reframing.
Continue contours at the same seam positions and directions, including street surfaces, wall bases and architectural edges. Match the reference palette, linework, brush or pixel treatment, texture density, lighting and object scale. Do not sharpen, add intricate detail, blur, or change the artistic style. Keep the background visually subordinate to gameplay. Exclude characters, text, UI, watermarks and smoke.`;

/** Upgrade only our exact obsolete defaults; authored prompts are never rewritten. */
export function upgradeLegacyOverallPrompt(prompt: string) {
  return [LEGACY_PLAN_VIEW_PROMPT, LEGACY_SIDE_VIEW_PROMPT].includes(
    prompt.trim(),
  )
