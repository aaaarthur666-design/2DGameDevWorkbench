...
import { CENTER_KEY, EMPTY_FEATHER, type SavedImageReference } from './map-types';
import {
...
export async function createPixelworkStatePackage(snapshot: FrameRoninEditorSnapshot) {
  const sourceTile = snapshot.tiles.find((tile) => tile.key === CENTER_KEY);
  const source = sourceTile?.images.overall;
  if (!sourceTile || !source) throw new Error('中心卡片缺少整体层，无法保存状态');
  ...
  const packBlob = (blob: Blob, preferredName: string, width: number, height: number): SavedImageReference => {
  ...
  const sourceReference = packBlob(source.file, `source_${source.name}`, source.width, source.height);
  ...
  for (const tile of snapshot.tiles) {
    if (tile.key !== CENTER_KEY && tile.images.overall) {
      ...
      tileUploads[tile.key] = packBlob(asset.file, `tile_${keyName(tile.key)}_${asset.name}`, asset.width, asset.height);
    }
    ...
    if (tile.images.overall && tile.images.object) {
      const mask = await deriveMaskCanvas(tile.images.overall, tile.images.object);
  ...
  const state: PixelworkMapStateV2 = {
    ...
    tiles: Object.fromEntries(snapshot.tiles.filter((tile) => tile.key !== CENTER_KEY).map((tile) => [
      tile.key,
      { x: tile.x, y: tile.y, w: tile.w, h: tile.h },
    ])),
    tileUploads,
    tileLayerUploads,
    tileFeathers: Object.fromEntries(snapshot.tiles.map((tile) => [tile.key, tile.feather])),
...
export async function downloadPixelworkState(snapshot: FrameRoninEditorSnapshot) {
  const result = await createPixelworkStatePackage(snapshot);
  downloadBlob(result.blob, result.fileName);
  return result;
}

export async function loadFrameRoninState(file: File): Promise<LoadedFrameRoninState> {
  const { manifest, zip } = await readFrameRoninStatePackage(file);
  if (isRecord(manifest) && manifest.format === 'pixelwork-map-stitch-state' && (manifest.version === 1 || manifest.version === 2)) {
    return loadPixelworkManifest(manifest, zip);
  }
  if (isRecord(manifest) && manifest.format === 'scenemaker-map-stitch-state' && manifest.version === 5) {
    return loadSceneMakerManifest(manifest, zip);
  }
  throw new Error('不是受支持的 Pixelwork v1/v2 或 SceneMaker v5 地图状态');
}

async function loadPixelworkManifest(manifest: Record<string, unknown>, zip: JSZip | null): Promise<LoadedFrameRoninState> {
  const sourceReference = normalizeReference(manifest.source, 'map_tile.png');
  if (!sourceReference) throw new Error('拼接状态缺少中心图片');
  const source = await loadReferenceAsset(sourceReference, zip, 'map_tile.png');
  const geometry = parsePixelworkGeometry(manifest.tiles);
  const tileUploads = isRecord(manifest.tileUploads) ? manifest.tileUploads : {};
  const rawLayerUploads = isRecord(manifest.tileLayerUploads) ? manifest.tileLayerUploads : {};
  const featherMap = isRecord(manifest.tileFeathers) ? manifest.tileFeathers : {};
  const hiddenMap = isRecord(manifest.hiddenPreviewTiles) ? manifest.hiddenPreviewTiles : {};
  ...
  for (const [key, bounds] of Object.entries(geometry)) {
    ...
    const overallReference = normalizeReference(tileUploads[key], `tile_${keyName(key)}.png`);
    if (overallReference) images.overall = await loadReferenceAsset(overallReference, zip, `tile_${keyName(key)}.png`);
    tiles.push({ key, ...bounds, images, feather: readFeather(featherMap[key]), hidden: hiddenMap[key] === true });
  }

  for (const layer of ['surface', 'object', 'black', 'white'] as const) {
    const byTile = isRecord(rawLayerUploads[layer]) ? rawLayerUploads[layer] : {};
    for (const tile of tiles) {
      const reference = normalizeReference(byTile[tile.key], `${layer}_${keyName(tile.key)}.png`);
      if (reference) tile.images[layer] = await loadReferenceAsset(reference, zip, `${layer}_${keyName(tile.key)}.png`);
    }
  }

  const shapes = parsePixelworkShapes(manifest.drawShapes ?? manifest.drawingShapes);
  const workbench = isRecord(manifest.workbench) ? manifest.workbench : {};
  const visibility = isRecord(workbench.layerVisibility) ? workbench.layerVisibility : {};
  const locks = isRecord(workbench.layerLocks) ? workbench.layerLocks : {};
  const regionVisibilityRecord = isRecord(workbench.regionVisibility) ? workbench.regionVisibility : visibility;

  return {
    tiles,
    shapes,
    selectedKey: typeof manifest.selectedKey === 'string' ? manifest.selectedKey : null,
    horizontalOverlapPercent: clampNumber(manifest.horizontalOverlapPercent, 15, 0, 50),
    verticalOverlapPercent: clampNumber(manifest.verticalOverlapPercent, 15, 0, 50),
    ...
    regionVisibility: mergeBooleanMap(DEFAULT_REGION_VISIBILITY, regionVisibilityRecord),
...
async function loadSceneMakerManifest(manifest: Record<string, unknown>, zip: JSZip | null): Promise<LoadedFrameRoninState> {
  if (!Array.isArray(manifest.tiles) || !manifest.tiles.length) throw new Error('SceneMaker 状态中没有地图卡片');
  const warnings = [
    '已从 SceneMaker v5 迁移：ground 映射为 surface，整体层由旧视觉层合成。',
  ];
  const records = manifest.tiles.filter(isRecord);
  const centerRecord = records.find((record) => record.key === CENTER_KEY) ?? records[0];
  const centerLayers = isRecord(centerRecord.layers) ? centerRecord.layers : {};
  const centerGround = normalizeReference(centerLayers.ground, 'source_ground.png');
  if (!centerGround) throw new Error('SceneMaker 状态的中心卡片缺少 ground 图片');
  const sourceProbe = await loadReferenceAsset(centerGround, zip, 'source_ground.png');
  ...
  for (const record of records) {
    ...
    const groundRef = normalizeReference(layers.ground, `${keyName(key)}_ground.png`);
    const objectRef = normalizeReference(layers.object, `${keyName(key)}_object.png`);
    const foregroundRef = normalizeReference(layers.foreground, `${keyName(key)}_foreground.png`);
    const blackRef = normalizeReference(layers.black, `${keyName(key)}_black.png`);
    const whiteRef = normalizeReference(layers.white, `${keyName(key)}_white.png`);
    hadForeground ||= Boolean(foregroundRef);

    const images: FrameRoninTile['images'] = {};
    const compositeAssets = [];
    if (groundRef) {
      images.surface = await loadReferenceAsset(groundRef, zip, `${keyName(key)}_surface.png`);
      compositeAssets.push(await loadReferenceAsset(groundRef, zip, `${keyName(key)}_overall_ground.png`));
    }
    if (objectRef) {
      images.object = await loadReferenceAsset(objectRef, zip, `${keyName(key)}_object.png`);
      compositeAssets.push(await loadReferenceAsset(objectRef, zip, `${keyName(key)}_overall_object.png`));
    }
    if (foregroundRef) compositeAssets.push(await loadReferenceAsset(foregroundRef, zip, `${keyName(key)}_overall_foreground.png`));
    if (blackRef) images.black = await loadReferenceAsset(blackRef, zip, `${keyName(key)}_black.png`);
    if (whiteRef) images.white = await loadReferenceAsset(whiteRef, zip, `${keyName(key)}_white.png`);
    if (compositeAssets.length) {
      images.overall = await composeLegacyOverall(compositeAssets, `${keyName(key)}_overall.png`);
    ...
    if (Array.isArray(record.collisions)) {
      ...
      for (const [index, collision] of record.collisions.entries()) {
        ...
        shapes.push(regionRectToShape({
          id: typeof collision.id === 'string' ? collision.id : `legacy_collision_${index}`,
          tileKey: key,
  ...
  if (hadForeground) warnings.push('旧 foreground 已烘焙进整体层；无法可靠推断 top 区域，因此未伪造顶层标注。');

  const activeMapLayer = mapLegacyLayer(manifest.activeLayer);
  const visibility = isRecord(manifest.layerVisibility) ? manifest.layerVisibility : {};
...
async function composeLegacyOverall(assets: Awaited<ReturnType<typeof loadReferenceAsset>>[], name: string) {
  ...
  for (const asset of assets) context.drawImage(await loadImage(asset.url), 0, 0, width, height);
  return blobToAsset(await canvasToBlob(canvas), name);
}

async function loadReferenceAsset(reference: SavedImageReference, zip: JSZip | null, fallbackName: string) {
...
async function readFrameRoninStatePackage(file: File) {
  if (!file.name.toLowerCase().endsWith('.zip')) return { manifest: JSON.parse(await file.text()) as unknown, zip: null };
  const zip = await JSZip.loadAsync(file);
  const manifestFile = zip.file('map_stitch_state.json');
  if (!manifestFile) throw new Error('压缩包中缺少 map_stitch_state.json');
  return { manifest: JSON.parse(await manifestFile.async('string')) as unknown, zip };
}

async function referenceToBlob(reference: SavedImageReference, zip: JSZip | null) {
  ...
  if (reference.dataUrl) {
    ...
    if (!response.ok) throw new Error(`无法读取状态图片：${reference.fileName}`);
    return response.blob();
  }
  throw new Error(`图片 ${reference.fileName} 没有可读取的数据`);
}

function normalizeReference(value: unknown, fallbackName: string): SavedImageReference | null {
  if (typeof value === 'string' && value.startsWith('data:')) {
    return { fileName: fallbackName, type: dataUrlType(value), size: 0, width: 0, height: 0, dataUrl: value };
  }
  ...
  return {
    fileName: typeof value.fileName === 'string' ? value.fileName : fallbackName,
    type: typeof value.type === 'string' ? value.type : (dataUrl ? dataUrlType(dataUrl) : 'image/png'),
...
export function parsePixelworkGeometry(value: unknown) {
  ...
  if (!Object.keys(result).length) throw new Error('拼接状态中的地图块数据无效');
  return result;
}
...
export function parsePixelworkShapes(value: unknown): RegionShape[] {
  ...
  return value.flatMap((candidate, index) => {
    ...
    const layer = REGION_LAYERS.includes(candidate.layer as RegionLayer) ? candidate.layer as RegionLayer : null;
    const mode = candidate.mode === 'rectangle' || candidate.mode === 'polygon' || candidate.mode === 'free' ? candidate.mode : null;
    if (!layer || !mode || !Array.isArray(candidate.points)) return [];
    const points = candidate.points.flatMap((point) => isRecord(point) && Number.isFinite(point.x) && Number.isFinite(point.y)
      ? [{ x: Number(point.x), y: Number(point.y) }]
      : []);
    if ((mode === 'rectangle' && points.length < 2) || (mode !== 'rectangle' && points.length < 3)) return [];
    return [{
      id: typeof candidate.id === 'string' && candidate.id ? candidate.id : `shape_${index}`,
      tileKey: typeof candidate.tileKey === 'string' ? candidate.tileKey : CENTER_KEY,
...
function readDisplayLayer(value: unknown): MapDisplayLayer {
  return MAP_DISPLAY_LAYERS.includes(value as MapDisplayLayer) ? value as MapDisplayLayer : 'overall';
}

function mapLegacyLayer(value: unknown): MapDisplayLayer {
  if (value === 'ground') return 'surface';
  if (value === 'object' || value === 'black' || value === 'white') return value;
  return 'overall';
}
...
function readPoint(value: unknown) {
  return isRecord(value) ? { x: finiteNumber(value.x, 0), y: finiteNumber(value.y, 0) } : { x: 0, y: 0 };
}

function mergeBooleanMap<T extends string>(defaults: Record<T, boolean>, values: Record<string, unknown>) {
  return Object.fromEntries(Object.keys(defaults).map((key) => [key, booleanValue(values[key], defaults[key as T])])) as Record<T, boolean>;
}

function uniqueImageName(used: Set<string>, candidate: string, mimeType: string) {
  const safeCandidate = safeFileName(candidate);
  const extension = /\.[a-z0-9]+$/i.exec(safeCandidate)?.[0] ?? mimeExtension(mimeType);
