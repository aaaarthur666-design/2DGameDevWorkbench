...
import { resolveMapGenerationProvider, validateMapApiKey } from '../map-generation-settings.mjs';
import { requestJson } from './http.mjs';
import { generateWithHunyuan, planHunyuanMapSize } from './hunyuan-map-image.mjs';
import { preserveMapTemplatePixels } from './map-generation-image.mjs';
import { planMapGenerationSize, mapOutputPrompt } from './map-generation-size.mjs';
...
export function validateMapStitcherInput(input) {
  const errors = [];
  if (input.operation === 'generate-origin') {
    if (typeof input.prompt !== 'string' || !input.prompt.trim() || input.prompt.length > 12000) errors.push('请填写 1–12000 字的地图原图提示词。');
    if (input.aspectRatio !== undefined && !['1:1', '3:2', '2:3'].includes(input.aspectRatio)) errors.push('原图画幅必须为 1:1、3:2 或 2:3。');
    if (Object.keys(input).some((key) => !['operation', 'prompt', 'provider', 'aspectRatio'].includes(key))) errors.push('原图生成只接收提示词、模型和画幅，不接收参考图片或扩图参数。');
...
export async function executeMapStitcher(context) {
  if (context.input.operation === 'generate-origin') return generateOrigin(context);
...
async function generateOrigin({ capability, input, outputDirectory }) {
  const provider = resolveMapGenerationProvider(capability.connector, input.provider, 'generate-origin');
  const adapter = { id: 'map-stitcher', operation: input.operation, ...(provider ? { provider: provider.id, model: provider.model } : {}) };
  if (!provider?.apiKey) return { status: 'awaiting_configuration', requiredEnvironment: provider?.apiKeyEnv || capability.connector.providerEnv, result: null, adapter };
  const aspectRatio = input.aspectRatio || '1:1';
  const sizes = { '1:1': '1024x1024', '3:2': '1536x1024', '2:3': '1024x1536' };
  const prompt = `${input.prompt.trim()}

Create one complete environment image for a 2D game map, with no interface, text labels, border or montage. Follow the requested art style and camera view. This image will be the center of a larger map; keep the environment coherent to the edges. Output aspect ratio: ${aspectRatio}.`;
  const diagnostics = { operation: input.operation, provider: provider.id, model: provider.model, aspectRatio, stage: 'requesting' };
  const generatedOutputNames = ['generation-diagnostics.json'];
  const save = () => writeFile(path.join(outputDirectory, 'generation-diagnostics.json'), JSON.stringify(diagnostics, null, 2));
  try {
    validateMapApiKey(provider.apiKey);
    await save();
    let bytes;
    if (provider.protocol === 'gemini-generate-content') {
      bytes = await generateWithGemini(provider, prompt, null, { aspectRatio, imageSize: '1K' });
    } else if (provider.protocol === 'hunyuan-tokenhub-image') {
      const [w, h] = aspectRatio.split(':').map(Number);
      bytes = await generateWithHunyuan(provider, prompt, planHunyuanMapSize(w, h));
    } else if (provider.protocol === 'openai-images-edits') {
      const response = await requestJson(provider.endpoint, {
        ...
        body: JSON.stringify({ model: provider.model, prompt, n: 1, size: sizes[aspectRatio], output_format: 'png' }), timeoutMs: 300_000,
      });
      if (typeof response?.data?.[0]?.b64_json !== 'string' || !response.data[0].b64_json) throw new Error('图片 API 没有返回原图数据。');
      bytes = Buffer.from(response.data[0].b64_json, 'base64');
    } else throw new Error('当前模型不支持地图原图生成。');
    diagnostics.stage = 'validating';
    await writeFile(path.join(outputDirectory, 'provider-response.bin'), bytes);
    generatedOutputNames.push('provider-response.bin');
    const png = await sharp(bytes, { limitInputPixels: 64_000_000 }).png().toBuffer();
    await writeFile(path.join(outputDirectory, 'generated-origin-raw.png'), png);
    generatedOutputNames.push('generated-origin-raw.png');
    const { width, height } = await sharp(png).metadata();
    diagnostics.returned = { width, height };
    const [w, h] = aspectRatio.split(':').map(Number);
    if (!width || !height || Math.abs(width / height - w / h) / (w / h) > 0.03) throw new Error('原图返回的画幅比例不符合请求；已保留返回图片，请检查任务产物。');
    await writeFile(path.join(outputDirectory, 'generated-origin.png'), png);
    generatedOutputNames.push('generated-origin.png');
    diagnostics.stage = 'completed';
    await save();
    return { status: 'completed', adapter, generatedOutputNames,
      result: { generatedOrigin: 'generated-origin.png', source: 'official-api', provider: provider.id, model: provider.model, width, height, aspectRatio, requiresAdoption: true } };
  } catch (error) {
    diagnostics.stage = 'failed';
    await save();
    return { status: 'failed', error: error instanceof Error ? error.message : String(error), adapter, generatedOutputNames };
  }
}
...
async function generateLayer({ capability, input, outputDirectory }) {
  ...
  const template = await sharp(source.buffer, { limitInputPixels: 64_000_000 }).metadata();
  const diagnostics = {
    provider: provider.id, model: provider.model,
    template: { width: template.width, height: template.height },
    requested: null, returned: null, stage: 'preparing',
  };
  const diagnosticName = 'generation-diagnostics.json';
  const generatedOutputNames = [diagnosticName];
  const saveDiagnostics = () => writeFile(path.join(outputDirectory, diagnosticName), `${JSON.stringify(diagnostics, null, 2)}\n`);
  try {
    validateMapApiKey(provider.apiKey);
    const output = provider.protocol === 'openai-images-edits'
      ? planMapGenerationSize(template.width, template.height)
      : provider.protocol === 'hunyuan-tokenhub-image' ? planHunyuanMapSize(template.width, template.height) : null;
    diagnostics.requested = output;
    const effectivePrompt = output ? mapOutputPrompt(input.prompt, output) : input.prompt;
    diagnostics.effectivePrompt = effectivePrompt;
    diagnostics.stage = 'requesting';
    await saveDiagnostics();
    const generated = provider.protocol === 'gemini-generate-content'
      ? await generateWithGemini(provider, effectivePrompt, source)
      : provider.protocol === 'openai-images-edits'
        ? await generateWithOpenAI(provider, effectivePrompt, source, output)
        : provider.protocol === 'hunyuan-tokenhub-image'
          ? await generateWithHunyuan(provider, effectivePrompt, output, source)
          : null;
    if (!generated) throw new Error(`Unsupported map generation protocol: ${provider.protocol}`);
    // Preserve the provider bytes before any geometry checks or compositing.
    const returned = await sharp(generated, { limitInputPixels: 64_000_000 }).metadata().catch(() => null);
    const extension = { png: 'png', jpeg: 'jpg', webp: 'webp' }[returned?.format] ?? 'bin';
    const rawName = `provider-response.${extension}`;
    await writeFile(path.join(outputDirectory, rawName), generated);
    generatedOutputNames.push(rawName);
    diagnostics.rawImage = rawName;
    diagnostics.returned = returned ? { width: returned.width, height: returned.height, format: returned.format } : null;
    diagnostics.stage = 'compositing';
    await saveDiagnostics();
    // The explicit size is a contract. Don't silently accept a provider ignoring it.
    if (output && (returned?.width !== output.width || returned?.height !== output.height))
      throw new Error(`图片 API 返回尺寸不符合请求：模板 ${template.width}×${template.height}，请求 ${output.width}×${output.height}，返回 ${returned?.width ?? '?'}×${returned?.height ?? '?'}。原始返回图片及尺寸记录已保存在任务产物中。`);
    const composite = await preserveMapTemplatePixels(source.buffer, generated);
    await writeFile(path.join(outputDirectory, 'generated-layer.png'), composite);
    generatedOutputNames.push('generated-layer.png');
    diagnostics.stage = 'completed';
    await saveDiagnostics();
    return {
      status: 'completed',
      result: {
        generatedLayer: 'generated-layer.png', source: 'official-api',
        provider: provider.id, model: provider.model, layer: input.layer, tile: input.tile,
        dimensions: { template: diagnostics.template, requested: diagnostics.requested, returned: diagnostics.returned },
        diagnostics: diagnosticName, rawImage: rawName,
      },
      generatedOutputNames,
      adapter: { id: 'map-stitcher', operation: input.operation, provider: provider.id, model: provider.model },
    };
  } catch (error) {
    // Only store our diagnostic stages and dimensions; upstream error bodies may contain secrets.
...
async function generateWithGemini(provider, prompt, source, imageConfig) {
  const payload = await requestJson(provider.endpoint, {
    ...
    body: JSON.stringify({
      contents: [{
        ...
        parts: [
          { text: prompt },
          ...(source ? [{ inline_data: { mime_type: source.mimeType, data: source.buffer.toString('base64') } }] : []),
        ],
      }],
      generationConfig: { responseModalities: ['IMAGE'], ...(imageConfig ? { responseFormat: { image: imageConfig } } : {}) },
    }),
    timeoutMs: 300_000,
  });
  const parts = Array.isArray(payload?.candidates)
    ...
    : [];
  const imagePart = parts.find((part) => !part?.thought && (part?.inlineData?.data || part?.inline_data?.data));
...
async function generateWithOpenAI(provider, prompt, source, output) {
  const form = new FormData();
  form.append('model', provider.model);
  form.append('prompt', prompt);
  form.append('size', output.size);
...
async function composeMap({ input, outputDirectory, repositoryRoot }) {
  ...
  if (engineTargets.includes('godot')) {
    ...
    await writeFile(
      ...
      await createGodotPackage(stitched, regions, width, height, state),
    );
...
async function createGodotPackage(stitched, regions, width, height, state) {
  const zip = new JSZip();
  zip.file('source_state.zip', state);
