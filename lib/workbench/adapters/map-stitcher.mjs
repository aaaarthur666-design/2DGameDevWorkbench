...
import { bearerHeaders, requestBinary, requestJson } from './http.mjs';
...
async function generateLayer({ capability, input, outputDirectory }) {
  ...
  const generated = await resolveGeneratedImage(image, url, token);
  const imagePath = path.join(outputDirectory, 'generated-layer.png');
  await sharp(generated.buffer).png().toFile(imagePath);
  return {
    status: 'completed',
    result: {
...
async function resolveGeneratedImage(value, connectorUrl, token) {
  const inline = decodeImageValue(value);
  if (inline) return { buffer: inline, source: 'inline' };

  let artifactUrl;
  let connector;
  try {
    connector = new URL(connectorUrl);
    artifactUrl = new URL(value, connector);
  } catch {
    throw new Error('Map generation image URL is invalid.');
  }
  if (artifactUrl.protocol !== 'http:' && artifactUrl.protocol !== 'https:') {
    throw new Error('Map generation image URL must use HTTP or HTTPS.');
  }
  if (artifactUrl.username || artifactUrl.password) {
    throw new Error('Map generation image URL must not contain credentials.');
  }
  if (artifactUrl.origin !== connector.origin) {
    throw new Error('Map generation image URL must use the connector origin.');
  }
  const { buffer, contentType } = await requestBinary(artifactUrl, {
    method: 'GET',
    redirect: 'manual',
    headers: {
      accept: 'image/*',
  ...
  if (contentType && !contentType.toLowerCase().startsWith('image/')) {
    throw new Error(`Map generation artifact returned ${contentType}, not an image.`);
  }
  return { buffer, source: 'remote-url' };
}
