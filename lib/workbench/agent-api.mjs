import { createProject, createObject, KINDS, normalizeProject } from '../../features/interactable-editor/contract.mjs';
...
export async function getConversationGuidance(manifest) {
  ...
  return {
    ...
    capabilityIds: agentCapabilities(manifest).map((capability) => capability.id),
    manualCapabilities: manifest.capabilities.filter((c) => c.agent?.mode === 'manual').map((c) => ({ id: c.id, name: c.name, viewPath: c.ui.route, reason: c.agent.reason })),
    createsTask: false,
  };
}
...
export async function getTaskResult(manifest, { taskId }) {
  ...
  return {
    ...
    viewPath: capability.ui?.route ? `${capability.ui.route}${task.capabilityId === 'interactable-editor' ? `?task=${encodeURIComponent(taskId)}` : ''}` : null,
  };
}
...
export function interactableTemplate(manifest, { kind = 'inspect', name } = {}) {
  if (!KINDS.includes(kind)) throw new Error('Unknown interactable kind.');
  const project = createProject();
  const object = createObject(kind);
  if (name) { project.name = name; object.displayName = name; }
