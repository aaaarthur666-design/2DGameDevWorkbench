...
async function scan(manifest) {
  ...
  for (const row of native?.data?.assets || []) {
    ...
    put({
      ...
      nativeId: row.id,
      editorPath,
      taskIds: jobTasks.get(row.jobId) || [],
      files: undefined,
    });
  }
...
async function detail(manifest, catalog, assetId) {
  ...
  if (a.origin === 'sprite-pipeline') {
    ...
    const task = catalog.taskById.get(a.taskId);
...
export async function readAssetPreview(manifest, assetId) {
  ...
  const f = await taskFile(manifest, catalog.taskById.get(a.taskId), a.primary);
  ...
  for (const id of new Set(assetIds)) {
    ...
    for (const { file, name } of files) {
      ...
      if (totalBytes + file.bytes > maxBytes)
        throw new Error('所选素材超过 128 MB，请减少选择后分批下载。');
      let bytes;
      if (source.origin === 'sprite-pipeline') {
        ...
        const resolved = await taskFile(
          manifest,
          catalog.taskById.get(source.taskId),
          file.key,
        );
