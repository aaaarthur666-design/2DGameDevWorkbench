...
export const ASSET_KINDS = ['character', 'animation', 'map', 'interactable', 'scene'];
...
export const assetTools = [
  [
    ...
    'assets',
    'List durable artwork, not tasks. Defaults to active assets; scope=trashed reads the recoverable asset recycle bin. Filter character/action/candidate count/index and sort by creation time for latest generation. One animation candidate is one asset. Follow nextOffset with snapshot; query is a name keyword, not a whole natural-language instruction. Browser-only drafts are not accessible here. Read-only, no generation.',
  ],
  [
    'workbench_get_asset',
    'asset',
