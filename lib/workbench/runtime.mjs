...
export function publicCapability(capability) {
  ...
  return {
    ...
    connector: {
      ...
      ...(connector.generationUrlEnv
        ? {
            generationUrlEnv: connector.generationUrlEnv,
            externalGenerationConfigured: Boolean(process.env[connector.generationUrlEnv]),
          }
        : {}),
    },
...
async function runLocalAdapter(manifest, capability, input) {
  ...
  try {
    ...
    task.adapter = execution.adapter;
    ...
    for (const value of execution.additionalOutputs ?? []) {
      if (typeof value === 'string' && !task.outputs.includes(value)) task.outputs.push(value);
    }
    task.outputs = [...new Set(task.outputs)];
    if (task.status === 'failed') task.error = execution.error ?? 'Adapter reported a failed task.';
    await persistTask(manifest, task);
    return {
      task,
      taskPath,
      ...(execution.requiredEnvironment ? { requiredEnvironment: execution.requiredEnvironment } : {}),
    };
  } catch (error) {
    task.status = 'failed';
    task.updatedAt = new Date().toISOString();
...
export async function listTasks(manifest, { limit = 50 } = {}) {
  const directory = path.resolve(repositoryRoot, manifest.workspace.taskDirectory);
  let names;
  try {
