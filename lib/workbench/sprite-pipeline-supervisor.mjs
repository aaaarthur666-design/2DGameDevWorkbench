...
export function resolveSpritePipelineTarget(environment = process.env) {
  ...
  const configuredUiUrl = environment.NEXT_PUBLIC_SPRITE_PIPELINE_UI_URL?.trim();
...
export async function waitForSpritePipeline(
  ...
  while (Date.now() < deadline) {
    ...
    if (child && child.exitCode !== null) {
      throw new Error(`SpritePipeline 在就绪前退出，退出码 ${child.exitCode}。`);
    }
  ...
  throw new Error(`SpritePipeline 在 ${Math.round(timeoutMs / 1_000)} 秒内未就绪。`);
}
