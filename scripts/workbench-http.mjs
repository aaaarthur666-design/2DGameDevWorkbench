  ...
  try {
    ...
    if (request.method === 'POST' && url.pathname === '/v1/tasks') {
      ...
      if (!body || typeof body !== 'object' || Array.isArray(body)) {
        sendJson(response, 400, { status: 'failed', error: 'Request body must be an object.' });
        return;
      }
      ...
      const result = summarizeTask(await runConnector(manifest, capability, body.input));
      const status = result.status === 'running' || result.status === 'awaiting_configuration' ? 202 : 200;
    ...
    const validation = message.startsWith('Input validation failed:') || message.startsWith('Unknown capability');
    sendJson(response, validation ? 400 : 500, { status: 'failed', error: message });
  }
});
...
server.listen(port, host, () => {
  process.stdout.write(`Workbench runtime bridge ready at http://${host}:${port}\n`);
});
...
async function sendArtifact(response, requestedPath) {
  if (!requestedPath || !requestedPath.replaceAll('\\', '/').startsWith('outputs/')) {
...
function readPort(value, fallback) {
  ...
  if (!Number.isInteger(parsed) || parsed < 1024 || parsed > 65535) {
    throw new Error('WORKBENCH_RUNTIME_PORT must be an integer between 1024 and 65535.');
  }
  return parsed;
}
